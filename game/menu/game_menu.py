...
class Button:
    """A simple button class for the game menu."""
    def __init__(self, text, pos, size, callback): # noqa: D107
...
def _load_and_exit_menu(state):
    """Load the level and exit the menu."""
    state['level'] = load_level()
    state['is_menu'] = False


def _save_and_exit_menu(state):
    """Save the current level and exit the menu."""
...
def create_menu_buttons(state):
    ...
    load_button = Button(
        "Load", (50, 400), (200, 50), lambda: _load_and_exit_menu(state))
    save_button = Button(
        "Save", (300, 400), (200, 50), lambda: _save_and_exit_menu(state))
    quit_button = Button(
        "Quit", (550, 400), (200, 50), lambda: pygame.quit())
    reset_game = Button(
        "New Game", (250, 500), (300, 50), lambda: _reset_game(state))
    return [load_button, save_button, quit_button, reset_game]


def menu(display, buttons, offset_x_y):
    """Render the game menu with buttons."""
